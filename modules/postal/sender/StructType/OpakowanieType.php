...
class OpakowanieType extends AbstractStructBase
{
    ...
    /**
     * Set typ value
     * @uses \app\modules\postal\sender\EnumType\TypOpakowanieType::valueIsValid()
     * @uses \app\modules\postal\sender\EnumType\TypOpakowanieType::getValidValues()
    ...
    public function setTyp(?string $typ = null): self
    {
        ...
        if (!\app\modules\postal\sender\EnumType\TypOpakowanieType::valueIsValid($typ)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \app\modules\postal\sender\EnumType\TypOpakowanieType', is_array($typ) ? implode(', ', $typ) : var_export($typ, true), implode(', ', \app\modules\postal\sender\EnumType\TypOpakowanieType::getValidValues())), __LINE__);
        }
        $this->typ = $typ;
