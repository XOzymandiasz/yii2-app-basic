...
class PrzesylkaShortType extends AbstractStructBase
{
    ...
    public static function validateCzynnosciUpustoweForArrayConstraintFromSetCzynnosciUpustowe(?array $values = []): string
    {
        ...
        foreach ($values as $przesylkaShortTypeCzynnosciUpustoweItem) {
            ...
            if (!\app\modules\postal\sender\EnumType\CzynnoscUpustowaType::valueIsValid($przesylkaShortTypeCzynnosciUpustoweItem)) {
    ...
    /**
     * Set status value
     * @uses \app\modules\postal\sender\EnumType\StatusType::valueIsValid()
     * @uses \app\modules\postal\sender\EnumType\StatusType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $status
     * @return \app\modules\postal\sender\StructType\PrzesylkaShortType
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\app\modules\postal\sender\EnumType\StatusType::valueIsValid($status)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \app\modules\postal\sender\EnumType\StatusType', is_array($status) ? implode(', ', $status) : var_export($status, true), implode(', ', \app\modules\postal\sender\EnumType\StatusType::getValidValues())), __LINE__);
        }
        $this->status = $status;
