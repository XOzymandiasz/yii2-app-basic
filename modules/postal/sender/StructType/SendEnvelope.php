...
class SendEnvelope extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \app\modules\postal\sender\StructType\PakietType[]
     */
     ...
     * @param \app\modules\postal\sender\StructType\PakietType[] $pakiet
     * @param int $idBufor
     * @param \app\modules\postal\sender\StructType\ProfilType $profil
     */
    public function __construct(?int $urzadNadania = null, ?array $pakiet = null, ?int $idBufor = null, ?\app\modules\postal\sender\StructType\ProfilType $profil = null)
    {
        $this
    ...
    public static function validatePakietForArrayConstraintFromSetPakiet(?array $values = []): string
    {
        ...
        foreach ($values as $sendEnvelopePakietItem) {
            ...
            if (!$sendEnvelopePakietItem instanceof \app\modules\postal\sender\StructType\PakietType) {
     ...
     * @throws InvalidArgumentException
     * @param \app\modules\postal\sender\StructType\PakietType $item
     * @return \app\modules\postal\sender\StructType\SendEnvelope
     */
    public function addToPakiet(\app\modules\postal\sender\StructType\PakietType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \app\modules\postal\sender\StructType\PakietType) {
