...
class AccompanyingDocumentsType extends AbstractStructBase
{
    ...
    /**
     * Set type value
     * @uses \app\modules\postal\sender\EnumType\AccompanyingDocumentsEnum::valueIsValid()
     * @uses \app\modules\postal\sender\EnumType\AccompanyingDocumentsEnum::getValidValues()
    ...
    public function setType(?string $type = null): self
    {
        ...
        if (!\app\modules\postal\sender\EnumType\AccompanyingDocumentsEnum::valueIsValid($type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \app\modules\postal\sender\EnumType\AccompanyingDocumentsEnum', is_array($type) ? implode(', ', $type) : var_export($type, true), implode(', ', \app\modules\postal\sender\EnumType\AccompanyingDocumentsEnum::getValidValues())), __LINE__);
        }
        $this->type = $type;
