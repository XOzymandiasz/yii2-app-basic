...
class PotwierdzenieDoreczeniaType extends AbstractStructBase
{
    ...
    /**
     * Set sposob value
     * @uses \app\modules\postal\sender\EnumType\SposobDoreczeniaPotwierdzeniaType::valueIsValid()
     * @uses \app\modules\postal\sender\EnumType\SposobDoreczeniaPotwierdzeniaType::getValidValues()
    ...
    public function setSposob(?string $sposob = null): self
    {
        ...
        if (!\app\modules\postal\sender\EnumType\SposobDoreczeniaPotwierdzeniaType::valueIsValid($sposob)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \app\modules\postal\sender\EnumType\SposobDoreczeniaPotwierdzeniaType', is_array($sposob) ? implode(', ', $sposob) : var_export($sposob, true), implode(', ', \app\modules\postal\sender\EnumType\SposobDoreczeniaPotwierdzeniaType::getValidValues())), __LINE__);
        }
        $this->sposob = $sposob;
