...
class ListWartosciowyKrajowyType extends PrzesylkaRejestrowanaType
{
    ...
    /**
     * Set format value
     * @uses \app\modules\postal\sender\EnumType\FormatType::valueIsValid()
     * @uses \app\modules\postal\sender\EnumType\FormatType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $format
     * @return \app\modules\postal\sender\StructType\ListWartosciowyKrajowyType
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\app\modules\postal\sender\EnumType\FormatType::valueIsValid($format)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \app\modules\postal\sender\EnumType\FormatType', is_array($format) ? implode(', ', $format) : var_export($format, true), implode(', ', \app\modules\postal\sender\EnumType\FormatType::getValidValues())), __LINE__);
        }
        $this->format = $format;
