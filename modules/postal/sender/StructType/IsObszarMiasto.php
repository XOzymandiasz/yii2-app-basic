...
class IsObszarMiasto extends AbstractStructBase
{
     ...
     * - maxOccurs: 500
     * @var \app\modules\postal\sender\StructType\ObszarAdresowyType[]
     */
    protected ?array $adres = null;
    /**
     * Constructor method for isObszarMiasto
     * @uses IsObszarMiasto::setAdres()
     * @param \app\modules\postal\sender\StructType\ObszarAdresowyType[] $adres
     */
    ...
    public static function validateAdresForArrayConstraintFromSetAdres(?array $values = []): string
    {
        ...
        foreach ($values as $isObszarMiastoAdresItem) {
            ...
            if (!$isObszarMiastoAdresItem instanceof \app\modules\postal\sender\StructType\ObszarAdresowyType) {
     ...
     * @throws InvalidArgumentException
     * @param \app\modules\postal\sender\StructType\ObszarAdresowyType $item
     * @return \app\modules\postal\sender\StructType\IsObszarMiasto
     */
    public function addToAdres(\app\modules\postal\sender\StructType\ObszarAdresowyType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \app\modules\postal\sender\StructType\ObszarAdresowyType) {
