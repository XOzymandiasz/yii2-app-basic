...
class GetEPOStatusResponse extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType[]
     */
     ...
     * @param \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType[] $epo
    ...
    /**
     * Get epo value
     * @return \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType[]
     */
    ...
    public static function validateEpoForArrayConstraintFromSetEpo(?array $values = []): string
    {
        ...
        foreach ($values as $getEPOStatusResponseEpoItem) {
            ...
            if (!$getEPOStatusResponseEpoItem instanceof \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The epo property can only contain items of type \app\modules\postal\sender\StructType\PrzesylkaEPOType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * Set epo value
     * @param \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType[] $epo
     * @return \app\modules\postal\sender\StructType\GetEPOStatusResponse
     ...
     * @param \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType $item
     * @return \app\modules\postal\sender\StructType\GetEPOStatusResponse
     *@throws InvalidArgumentException
     */
    public function addToEpo(\app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \app\modules\postal\modules\poczta_polska\sender\StructType\PrzesylkaEPOType) {
            throw new InvalidArgumentException(sprintf('The epo property can only contain items of type \app\modules\postal\sender\StructType\PrzesylkaEPOType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->epo[] = $item;
    ...
    public static function validateErrorForArrayConstraintFromSetError(?array $values = []): string
    {
        ...
        foreach ($values as $getEPOStatusResponseErrorItem) {
            // validation for constraint: itemType
            if (!$getEPOStatusResponseErrorItem instanceof \app\modules\postal\modules\poczta_polska\sender\StructType\ErrorType) {
